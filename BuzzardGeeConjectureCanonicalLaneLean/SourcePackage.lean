/-!
# Source package model for `buzzard-gee-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "210a4fc14baba9c8e96b9934b617f1f70c06d1ea"
def sourceCheckoutClean : Bool := true

def sourceFiles : List SourceFile := [
  { path := "scripts/bzg_closure_guard.py", sha256 := "f2268f0a4a2e0f3ef7688dcc66dfd49dc296153d6a163f5c10473faf27b2b7f3", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  { file := "scripts/bzg_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/bzg_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  { file := "scripts/bzg_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/bzg_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/bzg_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/bzg_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/bzg_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/bzg_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/bzg_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_compatibility\", \"sigma_local\", \"kappa_compact\", \"rho_rigidity\", \"automorphic_lock\"]", line := 21 },
  { file := "scripts/bzg_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_compatibility", status := "derived_numeric", formula := "c_core_raw * density_raw - e_core_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 4 / Appendix A", validation := "required_positive", componentKeys := ["c_core_raw", "density_raw", "e_core_raw"] },
  { group := "constants", key := "sigma_local", status := "derived_numeric", formula := "capture_floor_raw - transport_loss_raw - restart_loss_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 5 / Appendix B", validation := "required_positive", componentKeys := ["capture_floor_raw", "restart_loss_raw", "transport_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 5 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "automorphic_lock", status := "derived_numeric", formula := "c_transfer_raw * transfer_gain_raw - e_transfer_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix D", validation := "required_positive", componentKeys := ["c_transfer_raw", "e_transfer_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix E", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "automorphic_lock", value := "1.02951", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#automorphic_lock", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix D" },
  { key := "automorphic_transfer", value := "1.0315400000000001", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#automorphic_transfer", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix E" },
  { key := "kappa_compact", value := "0.8038585209003215", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 5 / Appendix C" },
  { key := "kappa_compatibility", value := "1.091838", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compatibility", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 4 / Appendix A" },
  { key := "kappa_geometric", value := "1.0950320000000002", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_geometric", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 6 / Appendix D" },
  { key := "sigma_local", value := "1.075", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_local", sourceSection := "paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md Section 5 / Appendix B" }
]
